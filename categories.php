						...
						<table class="table table-bordered table-hover">
							...
							<?php
								...
								if ($_SESSION['login_id'] == 1) {
									// If login_id is 1, display all data
									$cats = $conn->query("SELECT * FROM laundry_categories ORDER BY id ASC");
								} else {
									// Otherwise, display data for the specific user
									$user_id = $_SESSION['login_id'];
									$cats = $conn->query("SELECT * FROM laundry_categories WHERE user_id = $user_id ORDER BY id ASC");
								}
